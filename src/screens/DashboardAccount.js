...
export default function Dashboard({ navigation }) {
    ...
    AsyncStorage.getItem('@moneywall:token', (err, token) => {
      ...
      .then(json => {
        ...
        for(let i=0; i<json.accounts.length; i++) {
          totalAmount += json.accounts[i].Amount;
        }
        for(let i=0; i<json.accounts.length; i++) {
          var color = generateColor();
          resultGraph.push({
            value: Math.round(json.accounts[i].Amount*100/totalAmount),
            color: color,
          })
          result.push({
  ...
  return (
    ...
    <View style={styles.container}>
      <NavigationDashboard navigation={navigation}  page={'account'} style={styles.navbarHeader}/>
