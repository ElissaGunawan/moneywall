...
export default function questCompleted({ navigation }) {
  const [data, setData] = useState([]);
  const [noData, setNoData] = useState(false);
  const fetchQuestCompeletedSummary = () => {
  ...
  useEffect(() => {
    fetchQuestCompeletedSummary();
  }, []);
  return (
    <View style={styles.cover}>
      <NavbarHeader page={"Quest"}/>
    <View style={styles.container}>
      <NavigationQuest navigation={navigation} page={'questCompleted'} style={styles.navbarHeader}/>
      <KeyboardAvoidingView style={styles.content}> 
        {noData && <Text style={styles.noDataText}>Not completed all quests yet</Text>}
