...
export default function Dashboard({ navigation }) {
  ...
  const fetchIncomeSummary = () => {
    AsyncStorage.getItem('@moneywall:token', (err, token) => {
      console.log(token);
      fetch('http://ec2-54-255-181-27.ap-southeast-1.compute.amazonaws.com:3000/incomedashboard?start_date='+ formatDate(startDate) +'&end_date='+ formatDate(endDate), {
        method: 'GET',
        headers: {
      ...
      .then(json => {
        ...
        for(let i=0; i<json.incomes.length; i++) {
          totalAmount += json.incomes[i].Amount;
        }
        for(let i=0; i<json.incomes.length; i++) {
          var color = generateColor();
          resultGraph.push({
            value: Math.round(json.incomes[i].Amount*100/totalAmount),
            color: color,
          })
          result.push({
            name: json.incomes[i].AccountName,
  ...
  return (
    <View style={styles.cover}>
      ...
      <View style={styles.container}>
        <NavigationDashboard navigation={navigation} page={'income'} style={styles.navbarHeader}/>
